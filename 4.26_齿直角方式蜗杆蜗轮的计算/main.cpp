...
/*
	...
	蜗轮节圆径		Worm gear pitch diameter
...
// 分度圆柱导程角	Pitch cylinder lead angle
double calcPitchCylinderLeadAngle(double m_n, double z1, double d1) {
	double tmp = asin(m_n * z1 / d1);
	return radToDeg(tmp);
}

// 蜗轮节圆径		Worm gear pitch diameter
double calcWormGearPitchDiameter(double z2, double m_n, double gamma_deg){
	double gamma_rad = degToRad(gamma_deg);
	return z2 * m_n / cos(gamma_rad);
}

// 中心距	centerDistance
double calcCenterDistance(double d1, double d2, double x_n2, double m_n) {
	return (d1 + d2) / 2 + x_n2 * m_n;
}

// 齿顶高	tooth creast height
double* calcToothCreastHeight(double x_n2, double m_n) {
	double h_a1 = 1.00 * m_n;
	double h_a2 = (1.00 + x_n2) * m_n;
	double* h_a = new double[2] {h_a1, h_a2};
	return h_a;
}

// 全齿高	Full tooth height
double calcFullToothHeight(double m_n) {
	return 2.25 * m_n;
}

// 齿顶圆直径	tip diameter
double* calcTipDiameter(double d1, double d2, double h_a1, double h_a2, double m_n) {
	double d_a1 = d1 + 2 * h_a1;
	double d_a2 = d2 + 2 * h_a2 + m_n;
...
int main() {
	...
	double gamma_deg = calcPitchCylinderLeadAngle(m_n, z1, d1);
	double d2 = calcWormGearPitchDiameter(z2, m_n, gamma_deg);
	double a = calcCenterDistance(d1, d2, x_n2, m_n);

	double* h_a = calcToothCreastHeight(x_n2, m_n);
	double h_a1 = h_a[0];
	double h_a2 = h_a[1];

	double h = calcFullToothHeight(m_n);

	double* d_a = calcTipDiameter(d1, d2, h_a1, h_a2, m_n);
	double d_a1 = d_a[0];
	double d_a2 = d_a[1];
